...
function extractUrls(text, inactive) {
  ...
  var matchesArray = Array.from(text.matchAll(regExp));
  ...
  for (var i in matchesArray) {
    ...
    matches = matchesArray[i];

    if (!matches || !matches.length || matches[0][0] === '@') { //Ïðîâåðêà íà ñîâïàäåíèå ïåðâîãî ñèìâîëà äîáàâëåíà äëÿ èñêëþ÷åíèÿ ïî÷ò ñ íåñêîëüêî óðîâíåâûìè äîìåíàìè
      continue;
    }
    var offset = 0;
    if (!matches[4]) {
